...
<td>    
 ...
 <div id="TextBoxDiv1">
		 ...
		 <c:forEach var="app" items="${newSurvey.questions}" varStatus="idx">
            <p>
                <form:input path="questions[${idx.index}].QUESTION" /> <br>
                	 <c:forEach var="app" items="questions[${idx.index}].answers" varStatus="id">
                	 							${id.index}<br>
                	 	 <form:input path="questions[${idx.index}].answers[${id.index}]" /> <br>
                	 </c:forEach>
            </p>
        </c:forEach>
</div>
...
<input type='button' value='Remove Question' id='removeButton'></td></tr>
     ...
     <tr>
      ...
      &nbsp;&nbsp;
      <input type="button"  value="Back" onclick="javascript:go('viewAllSurvey');">
      </td>
     </tr>     
     </table>   
  </form:form>
